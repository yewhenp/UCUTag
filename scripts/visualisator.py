...
def animator(tree_heights, tree_file_widths, ext_data, tag_data, tag_data_new, data_key):
    fig = go.Figure(data=[
        ...
        go.Surface(z=np.asarray(tag_data_new[data_key]).reshape((len(tree_heights), len(tree_file_widths))),
...
def main():
    ...
    parser.add_argument("--path-tag-new", dest="path_tag_new", default="new_rez/tag_rez_new.json", type=str)
    ...
    with open(args.path_tag_new, "r") as file:
        tag_data_new = json.load(file)
    ...
    for data_key in ["create_dir_time_all", "create_files_time_all", "access_files_time_all",
        ...
        animator(tree_heights, tree_file_widths, ext_data, tag_data, tag_data_new, data_key)
