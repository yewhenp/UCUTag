...
{
    std::cout << "--> Getting The List of Files of " << path << std::endl;
    ...
    {
        filler( buffer, "file54", nullptr, 0 );
        filler( buffer, "file349", nullptr, 0 );
    }
...
{
    std::cout << "--> Trying to read " << path << " " << offset << " " << size << std::endl;

    std::string file54Text = "Hello World From File54!";
    std::string file349Text = "Hello World From File349!";
    ...
    size_t len = selectedText.size();
    if (offset < len) {
        if (offset + size > len)
            size = len - offset;
        memcpy(buffer, selectedText.c_str() + offset, size);
    } else
        size = 0;
